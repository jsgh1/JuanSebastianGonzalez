...
AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM boarding_pass bp WHERE bp.check_in_id = NEW.check_in_id) THEN
        INSERT INTO boarding_pass (check_in_id, boarding_pass_code, barcode_value, issued_at)
        VALUES (
            NEW.check_in_id,
            left('BP-' || replace(NEW.check_in_id::text, '-', ''), 40),
            left('BAR-' || replace(NEW.check_in_id::text, '-', '') || '-' || to_char(NEW.checked_in_at, 'YYYYMMDDHH24MISS'), 120),
            NEW.checked_in_at
        );
    END IF;
    RETURN NEW;
...
BEGIN
    IF EXISTS (SELECT 1 FROM check_in WHERE ticket_segment_id = p_ticket_segment_id) THEN
        RAISE EXCEPTION 'Ya existe check-in para el ticket_segment_id %', p_ticket_segment_id;
    END IF;

    INSERT INTO check_in (ticket_segment_id, check_in_status_id, boarding_group_id, checked_in_by_user_id, checked_in_at)
    VALUES (p_ticket_segment_id, p_check_in_status_id, p_boarding_group_id, p_checked_in_by_user_id, p_checked_in_at);
END;
$$;
```
...
BEGIN
    ...
    SELECT check_in_status_id INTO v_check_in_status_id FROM check_in_status ORDER BY created_at LIMIT 1;
    SELECT boarding_group_id INTO v_boarding_group_id FROM boarding_group ORDER BY sequence_no LIMIT 1;
    SELECT user_account_id INTO v_user_id FROM user_account ORDER BY created_at LIMIT 1;

    CALL sp_register_check_in(v_ticket_segment_id, v_check_in_status_id, v_boarding_group_id, v_user_id, now());
END;
$$;

SELECT ci.check_in_id, ci.ticket_segment_id, bp.boarding_pass_code, bp.issued_at
FROM check_in ci
